...
export default async function GradebookPage({
  ...
  const allScoresMap: Record<string, Record<string, number>> = {};
      ...
      if (lessons.length > 0) {
        ...
        if (submissions) {
          submissions.forEach((sub) => {
            ...
            if (!allScoresMap[sub.student_id])
              allScoresMap[sub.student_id] = {};
            const cur = allScoresMap[sub.student_id][sub.content_id] || 0;
            allScoresMap[sub.student_id][sub.content_id] = Math.max(
              cur,
              sub.score,
            );
          });
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: "100%",
                          backgroundColor:
                            progressPct === 100 ? "#10B981" : "var(--primary)",
                          borderRadius: "99px",
            ...
            {!params.student_id ? (
              ...
              >
                ...
                <div style={{ fontWeight: 600, marginBottom: "8px" }}>
                  Pilih murid dari daftar di kiri
                </div>
                <div style={{ fontSize: "0.875rem" }}>
                  Detail progres dan nilai murid akan ditampilkan di sini.
                </div>
              </div>
            ) : (
              <div>
                ...
                >
                  ...
                  <div style={{ textAlign: "right" }}>
                    {(() => {
                      const doneCount = lessons.filter(
                        (l) => (selectedStudentScores[l.id] ?? -1) >= 70,
                      ).length;
                      const progressPct =
                        totalLessons > 0
                          ? Math.round((doneCount / totalLessons) * 100)
                      ...
                      return (
                        <>
                          ...
                          >
                            {doneCount} dari {totalLessons} soal selesai
                          </div>
                        </>
                      );
                    })()}
