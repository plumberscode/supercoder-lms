...
export default function SubjectList({ initialSubjects }: SubjectListProps) {
  ...
  const handleSave = async (id: string) => {
    ...
    // Optimistic update
    const updatedSubjects = subjects.map(s => s.id === id ? { ...s, title: editTitle, description: editDescription } : s)
                      ...
                      {editingId === subject.id ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                          <input 
                            ...
                            className="input"
                            style={{ margin: 0, padding: '8px 12px', fontWeight: 600 }}
                          ...
                          <textarea 
                            ...
                            className="input"
                            style={{ margin: 0, padding: '8px 12px', minHeight: '60px', fontSize: '0.875rem' }}
                            placeholder="Deskripsi (Opsional)"
                          />
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <button onClick={() => handleSave(subject.id)} className="btn btn-primary" style={{ padding: '6px 12px', fontSize: '0.875rem' }}>Simpan</button>
                            <button onClick={() => setEditingId(null)} className="btn btn-secondary" style={{ padding: '6px 12px', fontSize: '0.875rem' }}>Batal</button>
                          </div>
                        </div>
                      ) : (
                        <>
                          ...
                          <p style={{ color: '#64748B', fontSize: '0.875rem', margin: 0 }}>{subject.description}</p>
                        </>
                      )}
                    </div>

                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                      <span className="badge" style={{ backgroundColor: subject.is_active ? '#DCFCE7' : '#F1F5F9', color: subject.is_active ? '#166534' : '#64748B' }}>
                        {subject.is_active ? 'Aktif' : 'Draft'}
                      </span>
                      <Link href={`/admin/subjects/${subject.id}`} className="btn btn-secondary" style={{ padding: '8px 16px', fontSize: '0.875rem' }}>
                        Kelola Modul →
                      </Link>
                    </div>
                  </div>
