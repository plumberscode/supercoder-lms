      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <select name="subject_id" required className="input" style={{ width: '100%' }}>
              {subjects.map(s => (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <input name="category" type="text" placeholder="Contoh: JavaScript Basic" required className="input" style={{ width: '100%' }} />
          </div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <label style={{ fontWeight: 600 }}>Teks Pertanyaan</label>
          <textarea name="question_text" required className="input" rows={3} style={{ width: '100%' }} placeholder="Tulis soal di sini..."></textarea>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <label style={{ fontWeight: 600 }}>Pilihan Jawaban (Pilih satu yang benar)</label>
          {options.map((opt, i) => (
