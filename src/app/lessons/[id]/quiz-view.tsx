    ...
    <div style={{ padding: '20px' }}>
      ...
      {!submitted ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          {questions.map((q, idx) => (
            <div key={q.id}>
              <p style={{ fontWeight: 600, marginBottom: '16px' }}>{idx + 1}. {q.question_text}</p>
                ...
                {q.options.map((opt: string, i: number) => (
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor: answers[q.id] === i ? 'var(--secondary)' : 'white',
        ...
        <div style={{ textAlign: 'center', padding: '40px' }}>
          ...
          <p style={{ color: '#64748B', marginTop: '8px' }}>
            {score >= (quiz.pass_score || 70) ? 'Bagus sekali! Anda lulus.' : 'Tetap semangat dan coba lagi ya!'}
          </p>
          <button className="btn btn-secondary" style={{ marginTop: '24px' }} onClick={() => window.location.reload()}>
            Coba Lagi
          </button>
        </div>
      )}
